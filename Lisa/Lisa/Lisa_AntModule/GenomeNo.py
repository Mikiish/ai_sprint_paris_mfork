...
class Genome:
    ###
    ### Représentation génétique des connexions d'un réseau de neurones.
    ### Chaque 'gène' dans 'connections' indique :
    ###   (source_neuron, target_neuron, weight)

    def __init__(self, connections=None, nb_inputs=0, nb_outputs=0):
        self.nb_inputs = nb_inputs  # Combien de neurones 'capteurs'
        self.nb_outputs = nb_outputs  # Combien de neurones 'sorties'
        # Si pas fourni, on initialise par exemple au hasard (ici, on reste simple).
        self.connections = connections if connections else []

    def mutate(self):
        ###
        ### Exemple simpliste : on peut altérer les poids ou ajouter/retirer une connexion.
        if not self.connections:
            return
        # On choisit aléatoirement un "gène" (connexion) et on modifie légèrement son poids.
...
class NeuralNet:
    ###
    ### Réseau de neurones concret, construit à partir d'un Genome.
    ...
    def __init__(self, genome: "Genome"):
        ...
        # Identifiants de neurones : on réserve
        # 0..(nb_inputs-1) pour les neurones d'entrée
        # nb_inputs..(nb_inputs+nb_hidden-1) pour les neurones cachés (s'il y en a)
        # (nb_inputs+nb_hidden)..(nb_inputs+nb_hidden+nb_outputs-1) pour neurones de sortie

        # Pour trouver le set complet de neurones potentiels, on regarde
        # toutes les connexions du génome, car il peut y avoir des neurones "cachés"
        self.all_neurons = set()
        for (s, t, w) in genome.connections:
            self.all_neurons.add(s)
            self.all_neurons.add(t)

        # On crée une structure d'adjacence pour pouvoir propager les signaux
        # Dict[ int -> List[ (target_neuron, weight) ] ]
        self.adj_list = defaultdict(list)
        for (s, t, w) in genome.connections:
            self.adj_list[s].append((t, w))

        # Les neurones d'entrée sont [0..nb_inputs-1]
        # Les neurones de sortie sont les derniers identifiants
        # (on suppose qu'ils suivent la suite, mais on peut définir autrement)
        ...
        # Calculer les neurones "utiles" (qui mènent à une sortie)
        self.useful_neurons = self._find_useful_neurons()

    def _find_useful_neurons(self):
        ###
        ### On part de chaque neurone de sortie, et on remonte dans les connexions
        ### pour trouver tous les neurones qui y mènent. Tout ce qui n'est pas visité
        ### ici est "inutile" (aucun chemin vers une sortie).
        ### """
        # Pour parcourir dans le sens inverse, on construit un graphe inversé
    ...
    def forward(self, inputs):
        ###
        ### On fait un calcul "feed-forward" très simple :
        ###   - On stocke les valeurs de chaque neurone dans un dict : neuron_values.
        ###   - Les neurones d'entrée prennent les 'inputs'.
        ###   - Ensuite, on parcourt (topologiquement, ou simplement en plusieurs passes)
        ###     en ne calculant que ce qui est "utile".
        ### Pour garder l'exemple court, on fait une approche itérative naïve :
        ### on recalcule tant qu'on peut mettre à jour un neurone.
        neuron_values = {}

        # 1) Initialiser les valeurs des neurones d'entrée
        for i, val in enumerate(inputs):
            neuron_values[i] = val

        # 2) Les neurones qui n'ont pas de valeur initiale commencent à zéro
        for n in self.all_neurons:
            if n not in neuron_values:
                neuron_values[n] = 0.0

        # 3) On effectue plusieurs itérations (ici un maximum arbitraire)
        #    pour propager les valeurs. On ne traite que les neurones "utiles".
        for _ in range(len(self.all_neurons)):
            for src in self.useful_neurons:
                val_src = neuron_values[src]
                # On envoie val_src * weight à chaque cible
                for (tgt, w) in self.adj_list[src]:
                    if tgt in self.useful_neurons:
                        neuron_values[tgt] += val_src * w

        # 4) Enfin, on récupère les valeurs de sortie
...
class Environment:
    ...
    def __init__(self, width=200, height=200, current_day=1):
        # On compte les jours de 1 à 365 (puis on pourrait boucler).
    ...
    def get_temperature(self, hour):
        ###
        ### Ex. : si hour est entre 7h et 19h => on est de jour
        ###       sinon de nuit
        ### On simplifie en fixant la température jour/nuit
        is_day = (7 <= hour < 19)
    ...
    def next_day(self):
        ...
        if self.current_day > 365:
            self.current_day = 1  # on boucle l'année
...
class Creature:
    ...
    def __init__(self, environment: Environment, name="Antoinette", max_lifespan=365):
        ...
        # Inventaire (simplifié) : dictionnaire [nom_de_molécule → quantité]
        self.inventory = {
            "food": 0,
            "sugar": 0,
            "pheromone": 0
        ...
        ### Initialisation du génome et du "cerveau"
    ...
    def new_day(self):
        ...
        self.age += 1
        self.meals_today = 0
        self.energy = (self.energy + 90) if self.age > 1 else 100.0
    ...
    def live_one_day(self, environment: Environment, speed_factor=1.0):
        ...
        # Parcours des 24h de la journée
        for hour in range(24):
            # Petit "sleep" pour ralentir la simulation
            time.sleep(1.0 / speed_factor)  # 1 sec par heure => 1 jour = 24 sec
                                            # si speed_factor=7 => ~3.4 sec/jour
            if self.energy <= 0:
                print(f"{self.name} est morte d'épuisement (energy=0).")
                self.is_alive = False
                return

            # Audit/log : on peut éventuellement afficher
            temp = environment.get_temperature(hour)
            if (hour % 4) == 0:
                print(f"[Jour {environment.current_day} - {hour}h] {self} - Temp={temp:.1f}°C")
