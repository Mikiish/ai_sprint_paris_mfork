...
def mutate_single(hex_number, mutation_index):
    """Effectue une mutation unique et vérifie la primalité."""
    ...
    return mutated_number, isprime(mutated_number)


def mutate_number(hex_number, max_mutations=100000, tolerance=2084, max_workers=4):
    """Effectue la mutation et la vérification de la primalité en parallèle."""
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        for i in range(max_mutations):
            mutation_index = random.randint(0, len(hex_number) - 1)
            future = executor.submit(mutate_single, hex_number, mutation_index)
            mutated_number, is_mutant_prime = future.result()
    ...
    return best_mutant if stable_mutations > 0 else None, stable_mutations, max_mutations, mutation_history
