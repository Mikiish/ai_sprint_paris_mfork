...
def mutate_number(hex_number, max_mutations=100000, tolerance=2084):
    ...
    for i in range(max_mutations):
        ...
        mutation_index = quantum_state.measure()  # Sélection de l’index via l'état quantique
        # casino_trigger_prime = casino_hex_prime()
        ...
        if isprime(mutated_number):
            ...
            quantum_state.reset()  # Réinitialisation après une mutation réussie
        else:
            consecutive_fails += 1
...
def inverse_mutation_search(n, max_steps=100):
